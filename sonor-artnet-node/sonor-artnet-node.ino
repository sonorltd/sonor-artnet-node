/*
 *  SONOR Art-Net Node  —  Ethernet → DMX512 out            v0.2.0
 *  Arduino Nano (ATmega328P) + ENC28J60 Nano Ethernet shield + MAX485 driver
 *
 *  Behaves like a single-port Enttec ODE:
 *    - Listens for Art-Net on UDP 6454
 *    - Answers ArtPoll with ArtPollReply so consoles / QLC+ / Control4 drivers discover it
 *    - Outputs ArtDmx for ONE universe to DMX on the hardware UART (TX / D1)
 *    - Built-in web page (http://<node-ip>/) to change Net / Sub-Net / Universe, DHCP or
 *      static IP, with wiring + flashing instructions. Settings persist in EEPROM.
 *    - DHCP first, falls back to a static Art-Net 2.x.x.x address if no DHCP within 5 s
 *    - Holds last look if the source stops sending (like an ODE)
 *
 *  Libraries are BUNDLED in ./src — nothing to install. Just open this .ino and Upload.
 *    src/EthernetENC  (Juraj Andrassy, patched)  — ENC28J60 driver with the standard Ethernet API
 *    src/DMXSerial    (Matthias Hertel)          — interrupt-driven DMX on the hardware UART
 ...
 *
 *  Wiring — see README.md or the node's own web page.
 */

#include <EEPROM.h>
#include "src/EthernetENC/EthernetENC.h"
#include "src/DMXSerial/DMXSerial.h"

// ─────────────────────────  FACTORY DEFAULTS  ─────────────────────────
// Everything here can be changed later from the node's web page.
...
#define DEF_IP            2, 0, 0, 10
#define DEF_MASK          255, 0, 0, 0
#define DEF_GW            2, 0, 0, 1

#define ETH_CS_PIN        10         // ENC28J60 chip-select (D10 on nearly all Nano shields; some use D8)
#define DMX_DE_PIN        2          // MAX485 DE+RE pin. Set to -1 if you tied DE/RE straight to 5V
#define LED_PIN           3          // Optional activity LED (D3 → 330Ω → LED → GND). D13 is SPI clock, don't use it.
...
const char LONG_NAME[]   PROGMEM = "SONOR Art-Net to DMX Node (Nano/ENC28J60) v" FW_VERSION;
...
const char HTML_HELP[] PROGMEM =
  ...
  "<h2>Flashing</h2><pre>"
  "Arduino IDE > open sonor-artnet-node.ino\n"
  "Board: Arduino Nano  |  Processor: ATmega328P (Old Bootloader) for clones\n"
  "Upload. Libraries are bundled in src/ - nothing to install.\n"

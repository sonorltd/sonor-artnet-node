...
button.g{background:#2b323a;color:#e6e8eb;margin-left:8px}
...
</style></head><body><div class=c><h1>SONOR Art-Net Node</h1><div class=k>%BOARD% &rarr; DMX512 &middot; firmware v%FW%</div>
%MSG%
<form action=/save method=post>
<h2>Art-Net</h2><div class=r>
<div><label>Net (0-127)</label><input name=n type=number min=0 max=127 value=%NET%></div>
<div><label>Sub-Net (0-15)</label><input name=s type=number min=0 max=15 value=%SUB%></div>
<div><label>Universe (0-15)</label><input name=u type=number min=0 max=15 value=%UNI%></div></div>
<div class=k style='font-size:12px;margin-top:6px'>Port-Address 0/0/0 is shown as "Universe 1" by most consoles; QLC+ and MA call it Universe 0.</div>
<h2>WiFi</h2><label>Network name (SSID)</label><input name=w value="%SSID%" maxlength=32>
<label>Password</label><input name=p type=password value="%PASS%" maxlength=64>
<div class=k style='font-size:12px;margin-top:6px'>%WIFINOTE%</div>
<h2>IP address</h2><label>Addressing</label><select name=d><option value=1 %D1%>DHCP (fall back to static below if none)</option><option value=0 %D0%>Static</option></select>
<label>Static IP</label><input name=a value=%IP%><label>Subnet mask</label><input name=m value=%MASK%><label>Gateway</label><input name=g value=%GW%>
<button>Save &amp; restart</button><button class=g formaction=/restart formmethod=post>Restart</button></form>
<h2>Status</h2><pre>Link      : %LINK%
IP        : %CURIP%  (%HOW%)
Hostname  : %HOST%.local
MAC       : %MAC%
Art-Net   : port 6454, Net %NET% / Sub-Net %SUB% / Universe %UNI%
Frames    : %FRAMES% received
DMX out   : 512 ch continuous, ~40 Hz</pre>
<h2>Wiring</h2><pre>%WIRING%</pre>
<h2>Flashing</h2><pre>Browser (no software) or Arduino IDE:
https://sonorltd.github.io/sonor-artnet-node/
Factory reset: set Net=127 Sub=15 Univ=15 and Save.</pre>
<div class=k style='font-size:13px'>Answers ArtPoll as "SONOR Node". Accepts ArtDmx by broadcast or unicast. Holds the last look if the source stops.
Source: github.com/sonorltd/sonor-artnet-node</div></div></body></html>)HTML";
